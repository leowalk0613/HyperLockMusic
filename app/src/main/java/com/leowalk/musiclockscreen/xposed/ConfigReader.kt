...
object ConfigReader {
    ...
    /** 生成壁纸 Bitmap 时使用的轻量 softColor 半径（重糊走系统合成器）。 */
    fun wallpaperBakeBlurRadius(context: Context): Float {
        refreshConfigIfNeeded(context)
        return SystemWallpaperBlurController.bakeBlurRadius(cachedBlurRadius)
    }

    /** 大专辑 Bitmap 暗色遮罩（轻量）；主暗角走系统 MiBlur mask。 */
    fun wallpaperBakeDarkOverlay(context: Context): Int {
        refreshConfigIfNeeded(context)
        return SystemWallpaperBlurController.bakeDarkOverlay(cachedDarkOverlay)
    }
